...
if __name__ == '__main__':
    while True:
        validVal= True
        try:
            val = float(input("Enter your value in bytes (numbers only): "))
        except:
            print("OOps, that's not numbers only. try again!")
            validVal= False
        if validVal:    
            print(friendly_bytes(val))
            k= input("That's your result! press E to Exit, and press C to get another friendly number")
            if k == 'E':
                break